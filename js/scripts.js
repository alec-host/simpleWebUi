	...
	Onyx = {
		...
		userFilesDropzone: {
			...
			initFileUploader: function(base, target) {
				...
				var mechanizeDropzone = new Dropzone(target, {

					url: ($(target).attr("action")) ? $(target).attr("action") : "../file_upload.php", // Check that our form has an action attr and if not, set one here
	                maxFiles: 1,
					maxFilesize: 20,
					acceptedFiles: ".csv",
					 ...
					 */
					dictDefaultMessage: "Drop file here to upload.", // Default: Drop files here to upload
				...
				mechanizeDropzone.on('dragenter', function () {
					$(target).addClass("hover");
				});

				mechanizeDropzone.on('dragleave', function () {
					$(target).removeClass("hover");			
				});

				mechanizeDropzone.on('drop', function () {
					$(target).removeClass("hover");	
				});

				mechanizeDropzone.on('addedfile', function () {
				...
				mechanizeDropzone.on('removedfile', function (file) {
				...
				mechanizeDropzone.on("success", function(file, response) {
